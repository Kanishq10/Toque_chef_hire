...
export function meta({}: Route.MetaArgs) {
  return buildMeta({ title: "Home Cooking Guides & Ideas | Toque", description: "Helpful guides on finding a home cook, planning meals, and hosting relaxed gatherings at home.", path: "/blogs", image: "/images/food-spread.png" });
}

export default function BlogsRoute() {
  const posts = [
    { title: '6 things to keep in mind when hiring a cook for home', tag: 'Home cooking', image: '/images/food-spread.png' },
    { title: 'How to plan a relaxed dinner party at home', tag: 'Entertaining', image: '/images/party-spread.png' },
    { title: 'Why fresh meals make busy weeks feel easier', tag: 'Wellness', image: '/images/cook-preparing.png' },
  ];
        ...
        <div className="mt-10 grid gap-6 sm:mt-14 md:grid-cols-3">
          {posts.map((post) => (
            <article key={post.title} className="overflow-hidden rounded-[--r-lg] border border-[#eadfce] bg-white shadow-sm">
              <img src={post.image} alt="" className="aspect-[4/3] w-full object-cover" loading="lazy" />
              <div className="p-6">
